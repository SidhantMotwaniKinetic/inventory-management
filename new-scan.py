...
def print_sticker(sticker_location: str):
    """Placeholder for printing sticker information."""
    if sticker_location:
        print(f"  Sticker Location: {sticker_location}")
    else:
        print("  No sticker location specified.")
...
def interactive_scan(expected: dict, reference: dict) -> None:
    ...
    while True:
        ...
        # supplier_id = scanned_id
        #for inventronics
        start_index = scanned_id.index('~') + 1
        end_index = len(scanned_id)
        supplier_id = scanned_id[start_index:end_index]
